...
class CellGraph3 {
 public:
  ...
  CellGraph3();
  CellGraph3(BoundingBox3 region, int maxLevel = 13);
   ...
   *
   * The order or the new nodes is the following:
   * Back: bottom left, bottom right, top left, top right.
   * Front: bottom left, bottom right, top left, top right.
   *
  ...
  /**
   * @brief For a given nodeId, the CellGraph3 strudcture queries the nodes
   ...
   ***************************************************************************
   *
   * @brief A Manager class for the CellGraph3 structure. All operations within
  ...
  class Nodes {
   public:
    /**
     * @brief Construct a new 3 object. The Nodes object is only created
     ...
     */
    size_t createNode(BoundingBox3 region, int level);
    size_t addNode(BoundingBox3 region, int level);
     ...
     */
    Eigen::Array3d getNodeCenterPosition(size_t nodeId);

    /**
     * @brief Get the Cell Region covered by the node with nodeId
     *
     * @return BoundingBox3 region covered by this node
     */
    BoundingBox3 getNodeRegion(size_t nodeId);
     ...
     * @param code
     */
    void setNodeAddressCode(size_t nodeId, Eigen::Array3i code);
     ...
     */
    Eigen::Array3i getNodeAddressCode(size_t nodeId);
   ...
   protected:
    ...
    std::vector<BoundingBox3> _cellsRegion;
    std::vector<Eigen::Array3i> _cellsAddressCode;
    ...
    std::vector<std::vector<Eigen::Array3d>> _vectorFields;
